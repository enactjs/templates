...
* `ri` _[object]_ - Resolution independence options to be forwarded to the [postcss-resolution-independence](https://github.com/enactjs/postcss-resolution-independence). By default, will use any preset for a specified theme or fallback to moonstone.
  * `baseSize` _[Number]_ - The root font-size to use when converting value of base unit to resolution-independent unit. When `baseSize` is set to 24, 48px in the LESS file will be converted to 2rem.
